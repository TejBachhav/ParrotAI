...
def preprocess_wav(fpath_or_wav: Union[str, Path, np.ndarray],
    ...
    # Apply the preprocessing: normalize volume and trim long silences 
    if normalize:
...
def trim_long_silences(wav):
    ...
    # Trim the audio so that its length is a multiple of the window size
    ...
    voice_flags = []
    vad_instance = webrtcvad.Vad(mode=3)
    for window_start in range(0, len(wav), samples_per_window):
        window_end = window_start + samples_per_window
        voice_flags.append(vad_instance.is_speech(pcm_wave[window_start * 2:window_end * 2],
